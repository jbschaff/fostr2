...
def login():
	...
	c = pycurl.Curl()
	#c.setopt(pycurl.URL, 'https://api.instagram.com/oauth/authorize/?client_id=9d836570317f4c18bca0db6d2ac38e29&redirect_uri=http://websta.me/&response_type=code&scope=comments+relationships+likes')
	c.setopt(pycurl.URL, 'https://www.instagram.com/accounts/login/?force_classic_login=&next=/oauth/authorize/%3Fclient_id%3D9d836570317f4c18bca0db6d2ac38e29%26redirect_uri%3Dhttp%3A//websta.me/%26response_type%3Dcode%26scope%3Dcomments%2Brelationships%2Blikes')
	...
	c.setopt(pycurl.SSL_VERIFYPEER, 0)
	c.setopt(pycurl.SSL_VERIFYHOST, 0)
	#c.setopt(pycurl.REFERER, "https://instagram.com/accounts/login/?next=/oauth/authorize/%3Fclient_id%3D"+clientid+"%26redirect_uri%3Dhttp%3A//websta.me/%26response_type%3Dcode%26scope%3Dlikes%2Bcomments%2Brelationships")
	c.setopt(pycurl.REFERER, "https://www.instagram.com/accounts/login/?force_classic_login=&next=/oauth/authorize/%3Fclient_id%3D"+clientid+"%26redirect_uri%3Dhttp%3A//websta.me/%26response_type%3Dcode%26scope%3Dcomments%2Brelationships%2Blikes")
	c.setopt(pycurl.USERAGENT, useragent)
...
def like():
	...
	while(True):
		...
		hashtaglikes = 0
		nextpage = "http://websta.me/tag/"+hashtags[current_tag]+"/?vm=list"
		...
		while nextpage != False and (hashtaglikelimit == 0 or (hashtaglikelimit > 0 and hashtaglikes < hashtaglikelimit)):
			...
			if len(nextpagelink)>0:
				nextpage = "http://websta.me/"+nextpagelink[0]
			else:
				nextpage = False
				break;
			...
			if len(likedata)>0:
				for imageid in likedata:
					...
					repeat = True
					while repeat:
						url = 'http://websta.me/api/like/'+imageid
						buf = cStringIO.StringIO()
						c = pycurl.Curl()
